...
- Target host: `THESIS`
- Target user: `thesis`
- Current hotspot IP: `10.129.205.32`
- Backend port: `8765`
...
The current hotspot IP used by the app defaults is `10.129.205.32`. If the
...
curl http://10.129.205.32:8765/api/status
```
...
async def main():
    async with websockets.connect("ws://10.129.205.32:8765/ws/thermal") as ws:
...
async def main():
    async with websockets.connect("ws://10.129.205.32:8765/ws/audio") as ws:
        for _ in range(5):
...
async def main():
    async with websockets.connect("ws://10.129.205.32:8765/ws/alerts") as ws:
        print(json.loads(await ws.recv()))
...
```bash
curl -X POST http://10.129.205.32:8765/api/alerts \
...
```bash
curl -X POST http://10.129.205.32:8765/api/leds \
  -H 'Content-Type: application/json' \
...
```bash
watch -n 0.5 curl -s http://10.129.205.32:8765/api/button
```

Flutter app endpoints:

```text
REST:    http://10.129.205.32:8765/api/
Thermal: ws://10.129.205.32:8765/ws/thermal
Audio:   ws://10.129.205.32:8765/ws/audio
Alerts:  ws://10.129.205.32:8765/ws/alerts
```
